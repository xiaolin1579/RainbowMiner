...
try {
	$PoolCoins_Request = Invoke-RestMethodAsync "https://pool.rwinfo.club/api/currencies" -tag $Name -cycletime 120 -delay 750 -timeout 30
}
catch {
...
try {
	$Pool_Request = Invoke-RestMethodAsync "https://pool.rwinfo.club/api/status" -tag $Name -cycletime 120 -delay 750 -timeout 30
}
catch {
...
$Pool_Currencies = @("DOGE") + @($PoolCoins_Request.PSObject.Properties | Where-Object {$_.Value.conversion_disabled -ne "1"} | Foreach-Object {if ($_.Value.symbol -eq $null){$_.Name} else {$_.Value.symbol}} | Select-Object -Unique) | Select-Object -Unique | Where-Object {$Wallets.$_ -or $InfoOnly}
		...
		foreach($Pool_SSL in ($false,$true)) {
			...
			foreach($Pool_Region in $Pool_Regions) {
				...
				[PSCustomObject]@{
					...
					Pass               = "{workername:$Worker},c=$Pool_ExCurrency,mc=$Pool_CoinSymbol{diff:,sd=`$difficulty}$Pool_Params"
